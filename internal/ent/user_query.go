...
func (uq *UserQuery) QueryExperiences() *WorkExperienceQuery {
	query := &WorkExperienceQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithExperiences(opts ...func(*WorkExperienceQuery)) *UserQuery {
	query := &WorkExperienceQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withJobs; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_jobs
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_jobs" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_jobs" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withExperiences; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Experiences = []*WorkExperience{}
		}
		query.withFKs = true
		query.Where(predicate.WorkExperience(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ExperiencesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_experiences
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_experiences" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_experiences" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Experiences = append(node.Edges.Experiences, n)
		}
	}
